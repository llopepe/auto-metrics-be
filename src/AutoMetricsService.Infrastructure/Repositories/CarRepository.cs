...
namespace AutoMetricsService.Infrastructure.Repositories
{
    public class CarRepository : BaseRepository<Car>, ICarRepository
    {
        ...
        public async Task<PaginatedList<Car>> GetAllPaginatedSearch(int page, int size, string search, string sortOrder, string sortDirection)
        {
            page = page <= 0 ? 1 : page;
            size = size <= 0 ? 10 : size;

            IQueryable<Car> query = _dbSet.AsNoTracking();
            ...
            // --- PAGINACIÓN ---
            return await PaginatedList<Car>.CreateAsync(query, page, size);
        }
